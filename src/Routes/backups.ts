import { RouteBody } from "../Auxiliar";
...
export default [
	...
	},
	{
		...
		code(ctx) {
			...
			if (!data) return ctx.send(404, "Backup not found");
			if (!table) {
				ctx.send(200, data);
			}
			if (table) {
	...
	},
	{
		...
		auth: false,
		code(ctx) {
			const { table, id, key } = ctx.body as {
				id?: string;
				table?: string;
				key?: string;
			};
			try {
				if (!id) throw new Error("No backup ID provided");
				if (!table) {
					const success = ctx.server.util.restoreBackup(id);
					if (!success) throw new Error("Unexpected error while backuping");
					ctx.send(200, {
						success,
						id,
					});
	...
	},
	{
		...
		code(ctx) {
			...
			try {
				...
				if (!backup) throw new Error("Invald ID");
				if (!table) {
					...
					if (!key) {
						...
						if (!success) throw new Error("Unexpected error while deleting a backup");
						ctx.send(200, {
							success,
							id,
							table,
